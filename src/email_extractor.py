...
class EmailExtractor:
    ...
    def __init__(self, config_path: str = "config/config.yaml"):
        ...
        self.db = EmailDatabase(
            self.config.get("database", {}).get("path", "data/database/emails.db")
        )
    ...
    def get_webpage_content(
        ...
        max_retries = (
            ...
            else self.config.get("extraction", {}).get("max_retries", 3)
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    "Попытка %s/%s не удалась для %s: %s",
                    attempt + 1,
                    max_retries,
                    url,
                    e,
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        "Не удалось загрузить страницу %s после %s попыток",
                        url,
                        max_retries,
                    )
    ...
    def extract_emails_from_webpage(
        self, url: str
    ) -> tuple[Literal[""], list[Any]] | list[str] | list[Any]:
        """
    ...
    def process_single_url(
        self, url: str, category: Optional[str] = None, company: Optional[str] = None
    ) -> List[str]:
        """
    ...
    def process_category(
        self, category_name: str, urls_file: str, output_file: Optional[str] = None
    ) -> List[str]:
        """
        ...
        for i, url in enumerate(urls, 1):
            ...
            try:
                ...
                delay = self.config.get("extraction", {}).get(
...
Примеры использования:
  ...
  python -m src.email_extractor --all-categories
    ...
    parser.add_argument(
        "--config", default="config/config.yaml", help="Путь к файлу конфигурации"
    )
    parser.add_argument(
        "--stats", action="store_true", help="Показать статистику извлечения"
    )
    ...
    try:
        ...
        if args.stats:
            ...
            if stats:
                ...
                for stat in stats:
                    ...
                    print(
                        f"URL: {stat['total_urls']}, Успешно: {stat['successful_extractions']}"
                    )
                    print(
                        f"Email: {stat['total_emails_found']}, Время: {stat['duration_seconds']:.1f}с"
                    )
                    print("-" * 40)
            else:
        ...
        if args.category:
            ...
            category_config = next(
                (c for c in categories if c["name"] == args.category), None
            )
